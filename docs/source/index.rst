Документация проекта DevTools!
==============================

Добро пожаловать! Здесь вы сможете найти всю необходимую информацию о том,
как воспользоваться замечательными классами из проекта DevTools.

Содержание
----------

.. toctree::
   :maxdepth: 2

   code/aleksey-kostrov/docs/dihotomy_search.rst
   code/aleksey-nesmelov/docs/fractcalc.rst
   code/alexandr-smirnov/docs/sort.rst
   code/andrey-golodyaev/docs/ComplexCalculator.rst
   code/anna-zhbanova/docs/Triangle.rst
   code/denis-sabanov/docs/AreaConverter.rst
   code/dmitriy-gorokhov/docs/QuadraticEquation.rst
   code/dmitry-vodopyanov/docs/lengthconvertor.rst
   code/ilia-udalov/docs/regex.rst
   code/kirill-kornyakov-lab2/docs/simplecalc.rst
   code/kirill-nikolaev/docs/NumConverter.rst
   code/ksenya-kochanova/docs/TempConv.rst
   code/mihail-kupriyanov/docs/mathquancalc.rst
   code/moshkov-yuriy/docs/DistanseBetweenVectors.rst
   code/pavel-korotaev/docs/WeightConvertor.rst
   code/vasiliy-kumaev/docs/currencyconvert.rst

Указатель и таблицы
-------------------

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
